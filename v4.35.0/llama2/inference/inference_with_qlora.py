...
def stream_generate(dialog, max_length, top_p, temperature):
    with torch.no_grad():
        ...
        while (out_tokens is None) or ((out_tokens[0][-1] != end_token) and (prompt_len + out_tokens.size()[1] < max_length)):
            ...
            cur_text = text[len(total_text):]
            total_text = text

            yield total_text, cur_text


for dialog, conversation in zip(dialog_list, conversation_list):
    for total_text, cur_text in stream_generate(dialog, 512, 0, 0):
        import json
        print(json.dumps({"text": total_text}))
        print(json.dumps({"text": cur_text}))
    break
